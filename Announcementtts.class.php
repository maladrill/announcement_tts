...
class Announcementtts extends \FreePBX_Helpers implements \BMO {
	...
	public function getAnnouncementstts() {
		$sql = "SELECT announcementtts_id, description, recording_id, allow_skip, post_dest, return_ivr, noanswer, repeat_msg FROM announcementtts";
	...
	public function getAnnouncementttsByID($id) {
		$sql = "SELECT announcementtts_id, description, recording_id, allow_skip, post_dest, return_ivr, noanswer, repeat_msg FROM announcementtts WHERE announcementtts_id = ?";
		$sth = $this->db->prepare($sql);
		$sth->execute([$id]);
